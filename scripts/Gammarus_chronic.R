...
# Remove strange row
summarised_data <- summarised_data[-2401,]  # TODO check how this row is made

# Convert into wide format, using 10s timebins as columns
summarised_data <- spread(summarised_data[2:4], group, mean.speed)

# Add treatments as rownames
rownames(summarised_data) <- summarised_data$Treatment_conc
# Remove treatments from df
